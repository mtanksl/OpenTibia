...
namespace OpenTibia.Plugins.Runes
{
    public class HeavyMagicMissileRunePlugin : RunePlugin
    {
        ...
        {
            var formula = Formula.GenericFormula(player.Level, player.Skills.GetClientSkillLevel(Skill.MagicLevel), 0.81, 4, 1.59, 10);

            return Context.AddCommand(new CreatureAttackCreatureCommand(player, target,

                new DamageAttack(ProjectileType.EnergySmall, null, DamageType.Energy, formula.Min, formula.Max, true) ) );
        }
    }
}
...
namespace OpenTibia.Plugins.Runes
{
    public class StoneShowerRunePlugin : RunePlugin
    {
        ...
        public override Promise OnUseRune(Player player, Creature target, Tile toTile, Item rune)
        {
            ...
            var formula = Formula.GenericFormula(player.Level, player.Skills.GetSkillLevel(Skill.MagicLevel), 1, 6, 2.6, 16);

            return Context.AddCommand(new CreatureAttackAreaCommand(player, false, toTile.Position, area, ProjectileType.SmallStone, MagicEffectType.Stones,

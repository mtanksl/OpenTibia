...
namespace OpenTibia.Plugins.Runes
{
    public class HolyMissileRunePlugin : RunePlugin
    {
        ...
        {
            var formula = Formula.GenericFormula(player.Level, player.Skills.GetSkillLevel(Skill.MagicLevel), 1.79, 11, 3.75, 24);

            return Context.AddCommand(new CreatureAttackCreatureCommand(player, target,

                new SimpleAttack(ProjectileType.Holy, MagicEffectType.HolyDamage, DamageType.Holy, formula.Min, formula.Max) ) );
        }
    }
}
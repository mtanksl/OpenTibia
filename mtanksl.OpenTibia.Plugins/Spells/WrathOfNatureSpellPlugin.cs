...
{
    public class WrathOfNatureSpellPlugin : SpellPlugin
    {
        public WrathOfNatureSpellPlugin(Spell spell) : base(spell)
        {
        ...
        public override Promise OnCast(Player player, Creature target, string message, string parameter)
        {
            ...
            var formula = Formula.GenericFormula(player.Level, player.Skills.GetClientSkillLevel(Skill.MagicLevel), 5, 0, 10, 0);
            ...
            return Context.AddCommand(new CreatureAttackAreaCommand(player, false, player.Tile.Position, area, null, MagicEffectType.PlantAttack, 
                ...
                new DamageAttack(null, null, DamageType.Earth, formula.Min, formula.Max, true) ) );
        }
    }
}
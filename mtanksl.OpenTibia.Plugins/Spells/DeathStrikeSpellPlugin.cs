        ...
        public override Promise OnCast(Player player, Creature target, string message, string parameter)
        {
            ...
            if (target == null)
            {
                ...
                return Context.AddCommand(new CreatureAttackAreaCommand(player, true, player.Tile.Position, area, null, MagicEffectType.MortArea,
                    ...
                    new DamageAttack(null, null, DamageType.Death, formula.Min, formula.Max, true) ) );
            }
            else
            {
                return Context.AddCommand(new CreatureAttackCreatureCommand(player, target,

                    new DamageAttack(ProjectileType.SuddenDeath, MagicEffectType.MortArea, DamageType.Death, formula.Min, formula.Max, true) ) );
            }
        }
    }
}
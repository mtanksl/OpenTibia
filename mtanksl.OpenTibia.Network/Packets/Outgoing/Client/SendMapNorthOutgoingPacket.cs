...
namespace OpenTibia.Network.Packets.Outgoing
{
    public class SendMapNorthOutgoingPacket : SendMapOutgoingPacket
    {
        ...
        public override void Write(IByteArrayStreamWriter writer, IHasFeatureFlag features)
        {
            ...
            if (Position.Z <= 7)
            {
                GetMapDescription(writer, features, Position.X - 8, Position.Y - 6, Position.Z, 18, 1, 7, -7);
            }
            else
            {
                GetMapDescription(writer, features, Position.X - 8, Position.Y - 6, Position.Z, 18, 1, Position.Z - 2, 4);
            }
        }
    }
}
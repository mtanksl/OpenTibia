    ...
    public class GreatManaPotionHandler : CommandHandler<PlayerUseItemWithCreatureCommand>
    {
        ...
        public override Promise Handle(Func<Promise> next, PlayerUseItemWithCreatureCommand command)
        {
            if (greatManaPotions.Contains(command.Item.Metadata.OpenTibiaId) && command.ToCreature is Player player)
            {
                ...
                {
                    return Context.AddCommand(new ShowTextCommand(player, MessageMode.MonsterSay, "Only sorcerers and druids of level 80 or above may drink this fluid.") );
                }
                ...
                return promise.Then( () =>
                {
                    return Context.AddCommand(new PlayerUpdateManaCommand(player, player.Mana + Context.Server.Randomization.Take(200, 300) ) );

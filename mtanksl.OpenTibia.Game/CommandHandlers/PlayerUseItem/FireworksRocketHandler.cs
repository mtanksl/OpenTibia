    ...
    public class FireworksRocketHandler : CommandHandler<PlayerUseItemCommand>
    {
        ...
        public override Promise Handle(Func<Promise> next, PlayerUseItemCommand command)
        {
            if (fireworksRockets.Contains(command.Item.Metadata.OpenTibiaId) )
            {
                ...
                {
                    return Context.AddCommand(new CreatureAttackCreatureCommand(null, command.Player, new SimpleAttack(null, MagicEffectType.ExplosionDamage, AnimatedTextColor.Orange, -10) ) );

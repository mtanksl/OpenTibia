...
namespace OpenTibia.Game.CommandHandlers
{
    public class TeleportPlayerHandler : CommandHandler<PlayerSayCommand>
    {
        ...
        {
            if (command.Message.StartsWith("/c ") && command.Player.Vocation == Vocation.Gamemaster)
            {
                ...
                if (observer != null && observer != command.Player)
                {
                    ...
                    if (toTile != null)
                    {
                        return Context.AddCommand(new CreatureUpdateTileCommand(observer, toTile) ).Then( () =>
                        {

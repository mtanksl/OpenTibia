...
namespace OpenTibia.Game.CommandHandlers
{
    public class TeleportToTownHandler : CommandHandler<PlayerSayCommand>
    {
        ...
        {
            if (command.Message.StartsWith("/t ") && command.Player.Vocation == Vocation.Gamemaster)
            {
                ...
                if (town != null)
                {
                    ...
                    if (toTile != null)
                    {
                        return Context.AddCommand(new CreatureUpdateTileCommand(command.Player, toTile) ).Then( () =>
                        {

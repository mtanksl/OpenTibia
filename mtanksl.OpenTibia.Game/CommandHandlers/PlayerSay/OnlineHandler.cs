    ...
    public class OnlineHandler : CommandHandler<PlayerSayCommand>
    {
        public override Promise Handle(Func<Promise> next, PlayerSayCommand command)
        {
            if (command.Message.StartsWith("!online") ) 
            {
                ...
                Context.AddPacket(command.Player, new ShowWindowTextOutgoingPacket(MessageMode.PrivateFrom, "Players online: " + players.Length) );

                for (int i = 0; i < players.Length; i += 10)
                {
                    Context.AddPacket(command.Player, new ShowWindowTextOutgoingPacket(MessageMode.PrivateFrom, string.Join(", ", players.Skip(i).Take(10).Select(p => p.Name + " [" + p.Level + "]") ) ) );
                }

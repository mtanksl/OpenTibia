...
namespace OpenTibia.Game
{
    public class MonsterFactory
    {
        ...
        public Monster Create(string name)
        {
            ...
            {
                server.Components.AddComponent(monster, new CreatureAttackThinkBehaviour(new DistantAttackStrategy(ProjectileType.ThrowingKnife, 2000, (attacker, target) => -server.Randomization.Take(0, 30) ) ) );

                server.Components.AddComponent(monster, new CreatureWalkThinkBehaviour(new KeepDistanceWalkStrategy(3) ) );
            }
            else if (monster.Name == "Valkyrie")
            {
                server.Components.AddComponent(monster, new CreatureAttackThinkBehaviour(new DistantAttackStrategy(ProjectileType.Spear, 2000, (attacker, target) => -server.Randomization.Take(0, 45) ) ) );

                server.Components.AddComponent(monster, new CreatureWalkThinkBehaviour(new KeepDistanceWalkStrategy(3) ) );
            }
            else if (monster.Name == "Deer")
            {
                server.Components.AddComponent(monster, new CreatureWalkThinkBehaviour(new RunAwayWalkStrategy() ) );
            }
            else if (monster.Name == "Dog")
            {
                server.Components.AddComponent(monster, new CreatureWalkThinkBehaviour(new ApproachWalkStrategy() ) );
            }
            else
            {
                server.Components.AddComponent(monster, new CreatureAttackThinkBehaviour(new CloseAttackStrategy(2000, (attacker, target) => -server.Randomization.Take(0, 20) ) ) );

                server.Components.AddComponent(monster, new CreatureWalkThinkBehaviour(new FollowWalkStrategy() ) );
            }

            if (monster.Metadata.Sentences != null)
            {
                server.Components.AddComponent(monster, new CreatureTalkThinkBehaviour(monster.Metadata.Sentences) );
            }

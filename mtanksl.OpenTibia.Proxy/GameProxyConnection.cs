...
namespace OpenTibia.Proxy
{
    public class GameProxyConnection : ProxyConnection
    {
        ...
        public GameProxyConnection(Logger logger, Socket socket, string host, int port) : base(socket, host, port)
        {
            this.logger = logger;
        }
        ...
        protected override void OnReceivedBodyFromClient(byte[] body)
        {
            ...
            try
            {
                ...
                if (identification == 0x0A)
                {
                    stream.Seek(Origin.Current, 5);
